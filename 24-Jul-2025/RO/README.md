...
**Title (ZH)**: JAM：分类意识边缘提议引导的关键点导向联合预测多智能体交互 
...
**Abstract (ZH)**: PRIX（基于原始像素的规划）：一种仅使用摄像头数据的高效端到端驾驶架构 
