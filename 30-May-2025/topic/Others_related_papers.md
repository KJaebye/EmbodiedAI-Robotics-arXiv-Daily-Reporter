...
**Title (ZH)**: 协同最后一英里配送：含沿途充电的多平台车辆路由问题 
...
**Title (ZH)**: 安全运营中心中基于可信赖自主性的统一的人工智能协作框架 
...
**Abstract (ZH)**: 因果推理对于业务过程干预和改进是必要的，要求对事件日志中活动执行时间之间的因果关系有清晰的理解。最近的研究引入了一种发现因果过程模型的方法，但缺乏捕捉多个变体中交替因果条件的能力。这提出了处理缺失值和在混合具有不同活动的日志片段时表达交替条件的挑战。我们提出了一种新颖的方法，将多个因果过程变体统一为一个一致的模型，同时明确表示它们的因果流变异性。该方法形式化定义、证明，并在三个公开和两个专有数据集上进行评估，开源实现已发布。 
...
**Abstract (ZH)**: 近年来，生成式AI模型的快速进步和普及化引发了关于安全、伦理风险和两用性担忧的重大讨论，特别是在网络安全领域的背景下。本文通过心理技术放大和赋能理论框架，提供实证证据分析生成式AI与恶意网络活动和网络犯罪之间的关联，并采用中断时间序列分析的准实验设计，研究两个数据集（一般性和加密货币相关）以实证评估生成式AI在网络安全犯罪中的角色。研究发现为AI治理的讨论增添了实证依据，平衡控制与促进创新，强调需要制定策略指导政策制定者、影响AI开发者和网络安全专业人员，并教育公众以最大限度地发挥AI的益处并减轻其风险。 

---
# COBRA: Contextual Bandit Algorithm for Ensuring Truthful Strategic Agents 

**Title (ZH)**: COBRA: 基于上下文的臂赛选算法以确保战略性代理的truthful行为 
...
**Abstract (ZH)**: 本文探讨了在低资源语言中构建高质量评估数据集的迫切需求，以促进跨语言迁移。通过多语言预训练利用跨语言迁移策略扩展语言技术至未研究和类型多样的语言，其效果依赖于高质量和合适的基准。本文发布了九种低资源语言的新感官注释数据集，这些语言跨多个语言家族和文字系统。为促进数据集的构建，本文提出了一种可证明有益的半自动注释方法。通过Word-in-Context (WiC) 格式实验展示了这些数据集在低资源语言迁移上的评估效果。结果强调了在低资源环境和迁移研究中针对性数据集创建和评估的重要性。所发布的数据集和代码旨在支持进一步研究公平、稳健且真正多语言的自然语言处理。 

---
# From Connectivity to Autonomy: The Dawn of Self-Evolving Communication Systems 

**Title (ZH)**: 从连接到自主：自进化通信系统的黎明 
...
**Abstract (ZH)**: 这篇论文构想了6G作为一个自我进化的电信生态系统，其中基于AI的智能使得动态适应超越了静态连接。我们探索自主通信系统的关键使能技术，涵盖可重构基础设施、自适应中间件、智能网络功能，并涉及分布式决策中的多代理协作。我们探讨了这些方法如何与新兴的工业物联网框架相契合，确保与数字化制造过程的无缝集成。我们的研究强调了在联网控制系统中增强实时决策、优化效率和降低延迟的潜力。讨论涵盖了伦理挑战、研究方向和标准化努力，并以技术栈路线图结束，以指导未来的发展。通过利用最先进的6G网络管理技术，本研究为下一代智能自动化解决方案做出了贡献，弥合了理论进步与实际工业应用之间的差距。 

---
# Skin Lesion Phenotyping via Nested Multi-modal Contrastive Learning 

**Title (ZH)**: 基于嵌套多模态对比学习的皮肤病变表型识别 
...
**Abstract (ZH)**: 在连接和自主车辆中，基于机器学习的安全消息分类对于检测恶意或异常行为已成为关键。然而，传统的依赖中心化数据收集或纯本地训练的方法由于车联网中固有的大规模、高移动性和异质数据分布而面临局限性。为克服这些挑战，本文探索了分布式联邦学习（DFL），该方法通过车辆之间交换模型更新并跨多跳传播模型以协作训练深度学习模型。使用Vehicular Reference Misbehavior（VeReMi）扩展数据集，我们展示无论对于何种车辆，DFL都可以显著提高分类准确性。值得注意的是，通过DFL，个体准确率较低的车辆实现了显著的准确率提升，这表明了网络中知识共享的好处。进一步的研究显示，局部训练数据大小和时间变化的网络连接性与模型的整体准确性密切相关。我们还在多个领域（包括无线 jamming 和训练数据投毒攻击）下研究了DFL的鲁棒性和脆弱性，结果揭示了DFL在面对多域攻击时的关键脆弱性，突显了在车联网中对DFL采取更稳健策略的必要性。 

---
# GSO: Challenging Software Optimization Tasks for Evaluating SWE-Agents 

**Title (ZH)**: GSO: 评估软件工程代理软件优化任务的挑战 
...
**Abstract (ZH)**: 基于密钥控制混沌动力学变换的新型安全和隐私保护神经网络推理框架 

---
# Securing AI Agents with Information-Flow Control 

**Title (ZH)**: 使用信息流控制保障AI代理的安全 
...
**Abstract (ZH)**: 随着AI代理越来越自主和强大，确保其免受诸如提示注入等漏洞的攻击变得至关重要。本文探讨了使用信息流控制（IFC）为AI代理提供安全保证的方法。我们提出了一种形式化模型来推理代理规划器的安全性和表达能力。通过该模型，我们确定了可由动态污点追踪强制执行的属性类别，并构建了任务分类法以评估规划器设计的安全性和实用性权衡。基于这一探索，我们提出了Fides规划器，该规划器追踪机密性和完整性标签，确定性地执行安全策略，并引入了有选择地隐藏信息的新颖机制。在AgentDojo中的评估表明，这种方法扩展了可以安全完成的任务范围。有关本文介绍的概念教程，请参阅此链接：https://example.com。 

---
# Human Empathy as Encoder: AI-Assisted Depression Assessment in Special Education 

**Title (ZH)**: 人类共情作为编码器：AI辅助的特殊教育中抑郁评估 
...
**Abstract (ZH)**: 评估敏感环境如特殊教育中的学生抑郁状况具有挑战性。标准化问卷可能无法全面反映学生的真实情况。此外，自动化方法往往难以处理丰富的学生叙事，缺乏来自教师对学生的同理心连接的关键性、个性化见解。现有方法常难以解决这种模糊性或有效整合教育者的理解。为了通过促进人类与人工智能的协同合作来解决这些局限，本文提出了一种新型、以人为核心的人工智能框架——Human Empathy as Encoder（HEAE），旨在实现透明和负责任的抑郁严重程度评估。我们的方法独特地将学生叙事文本与教师提取的9维“同理心向量”（EV）相结合，EV的各个维度由PHQ-9框架指导，以明确地将隐性的同理心洞察转化为结构化的AI输入，以增强而非替代人类判断。严格的实验优化了多模态融合、文本表示和分类架构，实现了7级严重程度分类的82.74%准确率。这项工作展示了通过结构化嵌入人类同理心实现更加负责任和道德的情感计算的道路。 

---
# AutoSchemaKG: Autonomous Knowledge Graph Construction through Dynamic Schema Induction from Web-Scale Corpora 

**Title (ZH)**: AutoSchemaKG：通过从大规模网页 corpora 中动态引申语 schemata 自主构建知识图谱 
...
**Abstract (ZH)**: 我们提出AutoSchemaKG，一种无需预定义模式的完全自主知识图谱构建框架 
...
**Abstract (ZH)**: 基于事件的多变量时序数据可解释处理管道：DataFul Explainable Multivariate Correlational Temporal Artificial Intelligence (EMeriTAte+DF) 

---
# Satori-SWE: Evolutionary Test-Time Scaling for Sample-Efficient Software Engineering 

**Title (ZH)**: Satori-SWE: 进化测试时动态缩放以提高样本高效软件工程 
...
**Abstract (ZH)**: Evolutionary Test-Time Scaling for Real-World Software Engineering Tasks 

---
# DeepChest: Dynamic Gradient-Free Task Weighting for Effective Multi-Task Learning in Chest X-ray Classification 

**Title (ZH)**: 深度胸部影像分类：动态无梯度任务加权方法以实现有效的多任务学习 
...
**Abstract (ZH)**: DeepChest：一种用于多标签胸部X光分类的高效动态任务加权框架 

---
# Engineering Serendipity through Recommendations of Items with Atypical Aspects 

**Title (ZH)**: 通过推荐具有非典型特征的物品实现偶然发现的工程 
...
**Abstract (ZH)**: 餐馆晚餐或酒店住宿可能因意外遇到符合个人兴趣的非典型元素而成为难忘的经历。例如，餐馆候餐区的手工折纸工作站可能会让热衷纸艺的顾客感到惊喜和愉快。同样，酒店大堂中展出的18世纪克拉维科特琴可能会让热衷巴洛克音乐的客人感到兴趣盎然。基于这一洞察，本文介绍了一种通过推荐具有非典型特性的物品来培养偶然性的新任务。我们描述了一种基于大语言模型（LLM）的系统流程，该流程从物品评论中提取非典型特性，然后估计并聚合这些特性的用户特定效用，将其用于重新排列推荐给用户的物品列表。为了促进系统开发和评估，我们引入了一个手工标注非典型特性的 Yelp 评论数据集和一个人工生成的用户画像数据集，以及用户-非典型特性效用值的众包标注。此外，我们引入了一种定制的上下文学习示例动态选择方法，该方法被证明可以提高基于大语言模型的非典型性与效用判断。实验评估表明，系统生成的基于偶然性的排名与从手动标注的非典型特性及其用户依赖效用计算出的真实排名高度相关。总体而言，我们希望通过本文介绍的新推荐任务和相关系统，激发进一步的研究，以探索超越准确性的推荐方法，追求增强的用户满意度。数据集和代码在此网址公开：this https URL。 

---
# CLaC at SemEval-2025 Task 6: A Multi-Architecture Approach for Corporate Environmental Promise Verification 

**Title (ZH)**: CLaC 在 SemEval-2025 任务 6 中的多模型方法及其在企业环境承诺验证中的应用 
...
**Abstract (ZH)**: 图表示学习（GRL）是机器学习中的一个基础任务，旨在将高维图结构数据编码为低维向量。自我监督学习（SSL）方法在GRL中广泛使用，因为它们可以避免昂贵的人工标注。在本文中，我们提出了一种新型的子图高斯嵌入对比（SubGEC）方法。我们的方法引入了一个子图高斯嵌入模块，该模块自适应地将子图映射到结构化高斯空间，同时保证输入子图特征的保留，并生成具有可控分布的子图。我们随后采用最优传输距离，即 Wasserstein 距离和 Gromov-Wasserstein 距离，以有效地度量子图之间的相似性，增强了对比学习过程的鲁棒性。在多个基准上的广泛实验表明，本方法在性能上优于或与最先进的方法相竞争。我们的发现为GRL中的SSL方法的设计提供了见解，强调了生成对比对之间分布的重要性。 
...
**Abstract (ZH)**: 排名系统在高 stakes 领域如健康、教育和就业中影响决策，这些领域可能产生重大经济和社会影响。因此，整合安全机制变得至关重要。其中一种机制是“弃权”（abstention），使算法决策系统能够将不确定或低置信度的决策交给人类专家。尽管弃权主要在分类任务中进行探讨，但其在其他机器学习范式中的应用尚未得到充分研究。在本文中，我们提出了在成对学习排序任务中弃权的一种新方法。我们的方法基于阈值化排序器的条件风险：当估计风险超过预定义阈值时，系统弃权决策。我们的贡献包括：最优弃权策略的理论表征、不依赖于特定模型的插件算法以构建弃权排序模型，以及在多个数据集上的全面实证评估，证明了我们方法的有效性。 

---
# From Knowledge to Noise: CTIM-Rover and the Pitfalls of Episodic Memory in Software Engineering Agents 

**Title (ZH)**: 从知识到噪声：CTIM-Rover及其软件工程代理中情景记忆的局限性 
...
**Abstract (ZH)**: CTIM-Rover：一种基于AutoCodeRover的软件工程AI代理 

---
# Toward Effective AI Governance: A Review of Principles 

**Title (ZH)**: 有效人工智能治理的原则综述 
...
**Abstract (ZH)**: 人工智能治理是建立框架、政策和程序以确保人工智能系统的负责任、伦理和安全开发与部署的实践。尽管人工智能治理是负责任人工智能的核心支柱，但当前文献仍缺乏对这类治理框架和实践的综合总结。目标：识别secondary文献中人工智能治理所强调的框架、原则、机制和利益相关者角色。方法：我们使用结构化的纳入标准和主题语义合成，对IEEE和ACM（2020-2024）的九篇同行评议secondary研究进行了快速三级审查。结果：引用最多的框架包括欧盟AI法案和NIST风险管理框架；透明度和问责制是最常见的原则。很少有综述详细说明可操作的治理机制或利益相关者策略。结论：该审查总结了人工智能治理的关键方向，并指出了实证验证不足和包容性不足的问题。研究结果既指导学术研究，也促进组织中的实际应用。 
...
**Abstract (ZH)**: 双向预测编码(bPC):结合生成和判别推理的生物可行性模型 

---
# Buffer-free Class-Incremental Learning with Out-of-Distribution Detection 

**Title (ZH)**: 无缓冲的类别增量学习与_out-of-distribution_检测 
...
**Abstract (ZH)**: 开放场景下类增量学习（CIL）提出了显著挑战，模型不仅要在线学习新类目同时不忘记先前的类目，还要处理来自封闭集模型无法正确分类的未知类别的输入。最新研究通过（i）使用任务增量学习框架训练多头模型，以及（ii）使用异常分布（OOD）检测器预测任务身份，同时有效应对了上述问题。然而，第二种方法主要依赖于与过去数据记忆缓冲的联合训练，这引发了关于隐私性、可扩展性和增加训练时间的担忧。本文深入分析了事后异常分布检测方法，并探讨了它们在消除记忆缓冲需求方面的潜力。我们发现，在合适的推理时间应用这些方法，可以作为基于缓冲的异常分布检测的强有力替代方案。实验结果表明，该无缓冲方法在类增量学习和排除未知样本方面，无论是性能还是表现，都达到了基于缓冲的方法的可比或优越水平。CIFAR-10、CIFAR-100和Tiny ImageNet数据集上的实验结果支持我们的发现，为开放场景下高效且隐私保护的CIL系统设计提供了新的见解。 

---
# VModA: An Effective Framework for Adaptive NSFW Image Moderation 

**Title (ZH)**: VModA：一种有效的自适应 NSFW 图像审核框架 
...
**Abstract (ZH)**: 蛋白质的局部结构强烈影响其功能及其与其他分子的相互作用。因此，对局部蛋白质环境进行简洁且信息丰富的表示对于蛋白质和生物分子相互作用的建模和设计至关重要。然而，这些环境的广泛结构和化学变异性使其建模具有挑战性，现有的表示形式仍未得到充分探索。在本工作中，我们提出了一种新的由原子尺度基础模型（AFMs）中间特征衍生的局部蛋白质环境表示方法。我们证明这种表示有效地捕捉了局部结构（如二级结构元件）和化学特征（如氨基酸 identities 和质子化状态）。进一步的研究表明，AFM衍生的表示空间具有有意义的结构，能够构建数据驱动的生物分子环境分布先验知识。最后，在生物分子核磁共振光谱学的背景下，我们展示了所提出的表示方法能够实现首屈一指的物理信息化学位移预测器，达到了最先进的准确性。我们的结果表明，原子尺度基础模型及其涌现表示在蛋白质建模中的有效性超出了传统的分子模拟。我们相信这将开启构建蛋白质环境有效功能表示的新途径。 

---
# Synthetic Generation and Latent Projection Denoising of Rim Lesions in Multiple Sclerosis 

**Title (ZH)**: Multiple Sclerosis中缘区病变的合成生成与潜空间去噪 
...
**Abstract (ZH)**: 基于工业标准的机器学习模型旨在满足快速变化的服务约束，这需要大量的资源进行模型开发。本文提出了一种名为MatTA的框架，利用新颖的Teacher-TA-Student配方训练多个准确的学生模型。TA模型是学生模型的扩展版本，具有更高的容量，使得学生模型能够更好地与教师模型相关，并引入更多特定领域的专业知识。此外，可以从TA模型中提取出多个准确的学生模型。因此，尽管只进行了一次训练运行，我们的方法仍提供了多种可部署选项，以在准确性与较低的服务成本之间进行权衡。我们在自有数据集和模型上展示了所提出的方法MatTA，并通过生产机器学习系统中的现场A/B测试，证明了该方法在一个关键指标上提高了20%。我们还在公开的GPT-2 Medium模型上展示了该方法，并在SAT Math和LAMBADA基准测试上分别实现了超过24%和10%的相对性能改进。 

---
# Enhancing Marker Scoring Accuracy through Ordinal Confidence Modelling in Educational Assessments 

**Title (ZH)**: 通过序级置信度建模提升标记评分准确性 
...
**Abstract (ZH)**: 基于评分生成模型的条件独立性检验方法：精确的I型错误控制与强大的测试能力 
...
**Abstract (ZH)**: FUSS：联邦学习框架下的无监督语义图像分割 
...
**Title (ZH)**: CheckThat! 2025深度检索：通过混合检索与再排序识别科学论文 
...
**Abstract (ZH)**: Deep Retrieval队列在CLEF CheckThat! 2025竞赛子任务4b中的方法与结果：基于社交媒体帖子检索相关科学文献 

---
# OSS-UAgent: An Agent-based Usability Evaluation Framework for Open Source Software 

**Title (ZH)**: OSS-UAgent：基于代理的开放源代码软件可使用性评估框架 
...
**Abstract (ZH)**: 开源软件（OSS）的可用性评估对于其影响和采纳至关重要，但传统的依赖人工评估者的方法存在着高成本和有限的可扩展性。为解决这些问题，我们引入了OSS-UAgent，这是一种自动化、可配置且交互式的基于代理的可用性评估框架，专门设计用于开源软件。该框架利用大型语言模型（LLMs）驱动的智能代理，模拟不同经验水平（从初级到专家）的开发者执行编程任务。通过动态构建平台特定的知识库，OSS-UAgent 确保代码生成的准确性和上下文感知性。生成的代码在多个维度上（包括合规性、正确性和可读性）自动评估，提供软件可用性的全面衡量。此外，我们的演示展示了OSS-UAgent 在评估图分析平台中的实际应用，突出了其在自动化可用性评估方面的有效性。 

---
# Less is More: Unlocking Specialization of Time Series Foundation Models via Structured Pruning 

**Title (ZH)**: 少即是多：结构化剪枝解锁时间序列基础模型的专业化 
...
**Abstract (ZH)**: 基于Scaling定律的时间序列基础模型（TSFMs）推动了大规模预训练参数的发展，并实现卓越的零样本预测性能。令人惊讶的是，即使经过微调，TSFMs也无法一致地战胜规模较小的下游全监督训练的专业模型。一个关键问题是如何实现TSFMs对目标预测任务的有效适应。通过各种TSFMs的实证研究，预训练模型通常表现出内在的计算稀疏性和冗余性，表明TSFMs已学会激活与多样化预测任务相关的网络子结构。为了保留这些宝贵的先验知识，我们提出了一种结构化剪枝方法，通过聚焦于更相关且紧凑的参数空间来规范化后续的微调过程。在七种TSFMs和六种基准上的广泛实验表明，对较小的剪枝TSFMs进行微调显著提高了预测性能，优于对原始模型进行微调。这种“先剪枝后微调”的范式通常能使TSFMs达到最佳性能并超越强大的专业基线。 

---
# Fooling the Watchers: Breaking AIGC Detectors via Semantic Prompt Attacks 

**Title (ZH)**: 愚弄监控者：通过语义提示攻击破解AIGC检测器 
...
**Abstract (ZH)**: 文本到图像模型的兴起使 photorealistic 人类肖像的合成成为可能，引发了关于身份滥用和 AIGC 检测器 robustness 的严重关切。本文提出了一种自动对抗式提示生成框架，该框架利用语法树结构和蒙特卡洛树搜索算法的变体，系统地探索语义提示空间。该方法生成多样且可控的提示，能够一致地规避开源和商业 AIGC 检测器。在多个文本到图像模型上的广泛实验验证了其有效性，并且该方法在实际的对抗式 AIGC 检测竞赛中排名第一。除了攻击场景之外，该方法还可以用于构建高质量的对抗式数据集，为训练和评估更加 robust 的 AIGC 检测和防御系统提供宝贵资源。 

---
# Unsupervised Word-level Quality Estimation for Machine Translation Through the Lens of Annotators (Dis)agreement 

**Title (ZH)**: 通过注释者（不）一致性的视角进行的机器翻译无监督词级质量估计 
...
**Abstract (ZH)**: 基于频率域的轻量高效 augmentation 方案：提升时间序列对比学习的下游分类性能 

---
# Best Arm Identification with Possibly Biased Offline Data 

**Title (ZH)**: 带有可能偏倚的离线数据的最佳臂识别 
...
**Abstract (ZH)**: 带潜在偏差的离线数据在固定置信区间下的最佳臂识别问题及其LUCB-H算法研究 

---
# VERINA: Benchmarking Verifiable Code Generation 

**Title (ZH)**: VERINA: 可验证代码生成的基准测试 
...
**Abstract (ZH)**: 听诊对于诊断肺部疾病至关重要。COVID-19疫情揭示了传统面对面肺音评估的限制。为克服这些问题，数字听诊器和人工智能的进步推动了新型诊断方法的发展。在此背景下，本研究旨在利用智能手机麦克风记录和分析肺音。我们面临两大挑战：电子听诊器与智能手机麦克风之间的音频风格差异，以及患者之间的变异。为应对这些挑战，我们开发了一种称为患者域监督对比学习（PD-SCL）的方法。通过将该方法与音频频谱变换器（Audio Spectrogram Transformer，AST）模型结合，我们将其性能提高了2.4%。这一进展表明，智能手机可以有效地诊断肺音，解决患者数据不一致的问题，并展现出在传统临床环境之外广泛使用的潜力。我们的研究为后 COVID-19 世界中的肺部疾病检测提供了更多机会。 

---
# ContextQFormer: A New Context Modeling Method for Multi-Turn Multi-Modal Conversations 

**Title (ZH)**: ContextQFormer: 一种新的多轮多模态对话上下文建模方法 
...
**Abstract (ZH)**: 有效的临床决策依赖于跨多样证据源进行迭代的多模态推理。近期出现的多模态推理模型显著改变了解决复杂任务的格局。尽管此类模型在数学和科学领域取得了显著成功，但其在医疗领域的应用仍然未被充分探索。在本文中，我们提出了一种名为MedE$^2$的两阶段后训练管道，用于激发并增强医疗领域的多模态推理能力。在第一阶段，我们使用包含精准推理示例的2,000个纯文本数据样本微调模型以激发推理行为。在第二阶段，我们进一步使用1,500个严格筛选的多模态医疗案例增强模型的推理能力，使模型输出与我们提出的多模态医疗推理偏好相匹配。广泛的经验表明，MedE$^2$在提升医疗多模态模型推理性能方面具备有效性和可靠性。值得注意的是，使用MedE$^2$训练的模型在多个医疗多模态基准测试中均优于基线模型。对更大模型的额外验证并在推理时间尺度下进一步确认了我们方法的鲁棒性和实用性。 

---
# Decom-Renorm-Merge: Model Merging on the Right Space Improves Multitasking 

**Title (ZH)**: Decom-Renorm-Merge: 在正确空间进行模型合并可提升多任务学习 
...
**Abstract (ZH)**: 基于外生变量的时间序列 Forecasting: 一种引入交叉相关嵌入模块的新型线性预测模型 

---
# EAD: An EEG Adapter for Automated Classification 

**Title (ZH)**: EAD：一种用于自动化分类的脑电图适配器 
...
**Abstract (ZH)**: SE(3)-共变图形神经网络（GNNs）通过使用群表示法显著推动了分子系统建模。然而，其依赖于张量积卷积的消息传递过程因非线性不足和不完整的群表示限制了其表达能力。为克服这些限制，我们引入了球变换共变框架（Equivariant Spherical Transformer, EST），该框架在傅里叶变换后的群表示空间域中采用Transformer结构。我们从理论上和实验上证明，EST能够涵盖张量积的功能空间，同时具有更高的表达能力。此外，EST通过傅里叶变换的均匀采样策略保证了其共变的归纳偏置。我们的实验结果显示，EST在OC20和QM9等多种分子基准测试中达到了最先进的性能。 

---
# Document-Level Text Generation with Minimum Bayes Risk Decoding using Optimal Transport 

**Title (ZH)**: 基于最优传输的最小贝叶斯风险解码的文档级文本生成 
...
**Abstract (ZH)**: 基于粒 Balls 的量子 $k$-最近邻算法 (GB-Q$k$NN): 一种高效的时间复杂度优化方法 

---
# Composite Flow Matching for Reinforcement Learning with Shifted-Dynamics Data 

**Title (ZH)**: 复合流匹配用于处理移位动力学数据的强化学习 
...
**Abstract (ZH)**: scaling规律基于Prior-data Fitted Networks的贝叶斯框架在深度学习中的扩展研究 

---
# Diverse Prototypical Ensembles Improve Robustness to Subpopulation Shift 

**Title (ZH)**: 多样原型集成提高子人群转移鲁棒性 
...
**Abstract (ZH)**: 子种群偏移导致训练集与目标集子种群分布差异，显著降低了机器学习模型的性能。针对子种群偏移的现有解决方案通过重新加权策略修改经验风险最小化，以提高泛化能力。该策略依赖于对子种群数量和性质的假设以及组成员身份的标注，而这些信息在许多真实世界的数据集中不可用。相反，我们提出使用多样化的分类器集合，以适应性地捕捉与子种群相关的风险。给定一个特征提取网络，我们将其标准的线性分类层替换为一种原型分类器的混合模型，其中每个成员都训练以分类数据并专注于不同的特征和样本。在涵盖不同领域和种类的子种群偏移的九个真实世界数据集上的实证评估中，我们所提出的方法Diverse Prototypical Ensembles (DPEs) 经常在最差组准确率方面优于之前的最佳方法。代码见此链接：https://xxx/ 

---
# $K^2$VAE: A Koopman-Kalman Enhanced Variational AutoEncoder for Probabilistic Time Series Forecasting 

**Title (ZH)**: $K^2$VAE：增强卡尔曼-科普曼变异自动编码器在概率时间序列预测中的应用 
...
**Abstract (ZH)**: 现代VLMs在英语文档视觉问答(VQA)任务中已接近饱和准确率。然而，由于缺乏合适的训练和评估数据，这一任务对于低资源语言仍然具有挑战性。本文介绍了针对匈牙利语（互联网上约第17位资源语言）的大规模数据集编纂方法。具体而言，我们提出了HuDocVQA和HuDocVQA-manual文档VQA数据集，与英语DocVQA相比，现代VLMs在这些数据集上的表现显著较差。HuDocVQA-manual基于Common Crawl中的手动编纂小型匈牙利文档数据集，而HuDocVQA是从相同来源生成的较大规模合成VQA数据集。我们对HuDocVQA进行多轮质量过滤和去重，以匹配该数据集的人类水平质量。我们还介绍了包含来自匈牙利Common Crawl PDF的11.7万页及其转录的HuCCPDF数据集，可用于训练匈牙利OCR模型。为了验证数据集的质量，我们展示了这些数据集的混合微调可以提高Llama 3.2 11B Instruct在HuDocVQA上的准确性，提高7.2%。我们的数据集和代码将对外开放，以促进多语言DocVQA的进一步研究。 
...
**Abstract (ZH)**: 稳健强化学习：HYDRO——跨域的混合稳健强化学习框架 
...
**Title (ZH)**: 图中验证：面向复杂声明验证的交互式图表示实体消歧改进 
...
**Abstract (ZH)**: Claim验证是一项长期且具有挑战性的任务，不仅要求高精度，还需要解释验证过程。随着大规模语言模型（LLMs）时代的到来，这一任务成为新兴的研究问题，因为实际的声明往往很复杂，包含复杂的语义结构或晦涩的实体。传统的做法通常通过将声明分解为子声明并查询知识库来解决隐藏或模糊的实体。然而，缺乏有效的消歧策略可能会破坏整个验证过程。为应对这些挑战，我们提出了基于图的验证（VeGraph）框架，该框架利用LLM代理的推理和理解能力。VeGraph分为三个阶段：（1）图表示 - 输入声明被分解为结构化的三元组，形成一种基于图的表示，整合了结构化和非结构化信息；（2）实体消歧 - VeGraph 通过迭代与知识库交互来解决图中的模糊实体，以进行更深层次的子声明验证；（3）验证 - 剩余的三元组被验证以完成事实核查过程。使用Meta-Llama-3-70B（指令版本）进行的实验表明，VeGraph 在两个基准 HoVer 和 FEVEROUS 上实现了竞争力的性能，有效解决了声明验证的挑战。我们的源代码和数据可供进一步利用。 

---
# Knowledge Distillation for Reservoir-based Classifier: Human Activity Recognition 

**Title (ZH)**: 基于水库模型的分类器知识蒸馏：人体活动识别 
...
**Abstract (ZH)**: 本文旨在通过引入PatchEchoClassifier新型模型来开发一种高效的时间序列数据分类器，该模型利用一种名为回声状态网络（ESN）的水库机制。该模型针对一维传感器信号的人体活动识别（HAR）进行设计，并融合了分词器以提取基于片段的表示。为了高效训练模型，我们提出了一种知识蒸馏框架，将高容量MLP-Mixer教师模型的知识转移到轻量级的ESN学生模型上。在多个HAR数据集上的实验评估表明，我们的模型能够实现超过80%的准确率，同时显著降低计算成本。值得注意的是，PatchEchoClassifier所需的浮点运算（FLOPS）数量仅为DeepConvLSTM（广泛使用的卷积基线）的六分之一。这些结果表明，PatchEchoClassifier是边缘计算环境中实时和高效的人体活动识别的一种有前景的解决方案。 

---
# EquiReg: Equivariance Regularized Diffusion for Inverse Problems 

**Title (ZH)**: 等变正则化的扩散模型用于逆问题 
...
**Abstract (ZH)**: 基于扩散的逆问题求解中的同胚正则化（EquiReg）方法：一种正则化扩散模型后验采样的通用框架 

---
# Exploring Scaling Laws for EHR Foundation Models 

**Title (ZH)**: 探索电子健康记录基础模型的扩展规律 
...
**Abstract (ZH)**: 电子健康记录（EHR）基础模型的标度律研究：从大规模语言模型中汲取经验 

---
# Learning to Charge More: A Theoretical Study of Collusion by Q-Learning Agents 

**Title (ZH)**: 基于Q-learning代理的合谋理论研究：学会索取更多 
...
**Abstract (ZH)**: 无限重复博弈中$q$学习代理learn supracompetitive价格的首个理论解释 

---
# HiLDe: Intentional Code Generation via Human-in-the-Loop Decoding 

**Title (ZH)**: HiLDe：基于人类在环解码的意图代码生成 
...
**Abstract (ZH)**: AI编程工具虽有潜力大幅提高程序员的能力和 productivity，但往往排除了用户在关键决策过程中的参与，导致许多人 effectively “关闭大脑”，过度依赖这些系统的解决方案。这些行为在软件安全等关键领域可能会造成严重后果。我们提出了一种名为 Human-in-the-loop Decoding 的新型交互技术，允许用户在代码生成过程中观察并直接影响 LLM 的决策，以使模型的输出与用户个人需求一致。我们在 HiLDe 中实现了这一技术，这是一种代码补全助手，突出显示了 LLM 作出的关键决策，并提供了局部替代方案供用户探索。在一项关于安全相关任务的单被试内研究（N=18）中，我们发现 HiLDe 使参与者生成的漏洞显著减少，并且更符合用户的目标。 

---
# Defining Foundation Models for Computational Science: A Call for Clarity and Rigor 

**Title (ZH)**: 定义计算科学中的基础模型：寻求清晰与严谨 
...
**Abstract (ZH)**: 我们研究了静态非线性反馈由前向神经网络（FFNNs）实现的Lur'e形式非线性系统的局部稳定性。通过利用正系统约束，我们采用Aizerman猜想的局部变体，为限制在紧致集内的轨迹的指数稳定性提供了充分条件。基于此基础，我们发展了两种不同的方法来估计吸引域（ROA）：（i）一种较少保守的基于李雅普诺夫的方法，构建二次函数的不变次水平集，满足线性矩阵不等式（LMI）；（ii）通过逐层传播线性放松来计算FFNNs的紧局部扇区界的新技术。这些界值集成到局部化的Aizerman框架中，以验证局部指数稳定性。数值结果表明，与现有的基于积分二次约束的方法相比，在ROA大小和可扩展性方面均实现了显著改进。 

---
# Scaling Offline RL via Efficient and Expressive Shortcut Models 

**Title (ZH)**: 通过高效且表达能力强的捷径模型扩展 Offline RL 
...
**Abstract (ZH)**: 基于流匹配的流式立体声语音合成框架：BinauralFlow 

---
# RocqStar: Leveraging Similarity-driven Retrieval and Agentic Systems for Rocq generation 

**Title (ZH)**: RocqStar: 利用相似性驱动的检索和能动系统进行Rocq生成 
...
**Abstract (ZH)**: 交互定理证明与生成人工智能的结合屡次显示出其有效性。本文评估了多种Rocq生成方法，并阐明了改进的潜在途径。我们强调了生成Rocq证明时彻底选择前提的重要性，并提出了一种新颖的方法，利用自我注意嵌入模型进行检索。设计方法的评估显示生成器性能相对提高了28%。我们使用一种针对形式验证定制的多阶段代理系统来解决编写Rocq证明的问题，并展示了其高度有效性。我们进行了消融研究，并展示了在证明合成规划阶段使用多代理辩论的应用。 

---
# Security Benefits and Side Effects of Labeling AI-Generated Images 

**Title (ZH)**: AI生成图像标记的安全优势与副作用 
...
**Abstract (ZH)**: 生成式人工智能正迅速发展，影响着人类与信息和数字媒体的交互方式。它被日益用于创造误导性且看似真实的虚假信息，因此立法者已制定法规要求披露AI生成的内容。然而，对于这些标签是否能减少AI生成的虚假信息的风险知之甚少。
...
**Title (ZH)**: aurora: Android恶意软件分类器在分布转移情况下是否可靠？ 
...
**Abstract (ZH)**: 现代漂移适应性恶意软件分类器的性能指标看起来很有前景，但这种性能是否转化为实际操作中的可靠性能？标准的评估范式主要关注基线性能指标，忽视了置信度误差的一致性和操作稳定性。虽然TESSERACT确立了时间评价的重要性，我们则采取了补充性的方法，探讨在分布迁移的情况下恶意软件分类器是否能够维持可靠的置信度估计，并探索科学进步与实际影响之间的张力。我们提出了AURORA框架，基于置信度质量和操作弹性来评估恶意软件分类器。AURORA通过对给定模型的置信度概貌进行验证，评估其估计的可靠性。不可靠的置信度估计会削弱操作中的信任度，浪费宝贵的标注预算在非信息性的样本上进行积极学习，并在选择性分类中未能检测到错误实例。AURORA进一步通过设计用于超越单一时间点性能的指标集，力求在时间评价期间实现对操作稳定性更具综合性的评估。我们观察到最先进的框架在不同漂移严重程度数据集上的脆弱性，表明需要回到“白板”重新审视。 

---
# How Do Diffusion Models Improve Adversarial Robustness? 

**Title (ZH)**: 扩散模型如何提高对抗robust性？ 
...
**Abstract (ZH)**: 本文通过全面分析分布转移与AI安全的概念和方法协同作用，架起了二者的桥梁。尽管以往讨论往往集中在狭窄的案例或非正式的类比上，我们建立了两种特定分布转移原因与细粒度AI安全问题之间的连接：（1）针对特定转移类型的方法可以有助于实现相应的安全目标，或（2）某些转移和安全问题可以形式上相互归约，从而使它们的方法能够相互适应。我们的发现提供了一个统一的视角，促进分布转移与AI安全研究的根本整合。 

---
# PGLearn -- An Open-Source Learning Toolkit for Optimal Power Flow 

**Title (ZH)**: PGLearn -- 一种开源最优功率 flow 学习工具包 
...
**Abstract (ZH)**: 机器学习（ML）技术在最优功率流（OPF）问题中的应用近年来引起了广泛关注，反映了更广泛的利用ML近似和/或加速解决复杂优化问题的趋势。这些进展是因为现代和未来电网中能源生产波动性和规模的增加而变得必要。然而，ML在OPF中的进展受到标准化数据集和评估指标缺乏的阻碍，从生成和解决OPF实例到训练和基准测试机器学习模型。为了解决这一挑战，本文提出了PGLearn，这是一个全面的标准化数据集和评估工具套件，用于ML和OPF。PGLearn提供的数据集代表了现实运行条件，通过明确捕捉数据生成中的全局和局部变异性，并首次包括多个大型系统的时间序列数据，从而支持多种OPF公制，包括AC、DC和二阶锥公制。标准化数据集对公众开放，旨在普及该领域的访问权，减轻数据生成的负担，并enable多种方法的公平比较。PGLearn还包含一个用于训练、评估和基准测试OPF机器学习模型的稳健工具包，旨在标准化领域内的性能评估。通过推广开放和标准化的数据集和评估指标，PGLearn旨在促进和加速最优功率流问题中机器学习应用的研究与创新。数据集可在此链接下载：这个 https URL。 

---
# IMTS is Worth Time $\times$ Channel Patches: Visual Masked Autoencoders for Irregular Multivariate Time Series Prediction 

**Title (ZH)**: IMTS 值得时间 $\times$ 通道 patches：视觉掩蔽自编码器在不规则多元时间序列预测中的应用 
...
**Abstract (ZH)**: 不规则多变量时间序列（IMTS）的预测因多通道信号的不齐和平坦数据的普遍存在而具有挑战性。现有的方法难以捕捉此类数据中的可靠时间模式，因为存在大量的缺失值。虽然预训练的基础模型具有解决这些挑战的潜力，但它们通常设计用于规则采样时间序列（RTS）。受视觉Mask AutoEncoder（MAE）对稀疏多通道信息建模的强大能力及其在RTS预测中的成功启发，我们提出了一种VIMTS框架，该框架将视觉MAE适应于IMTS预测。为了减轻缺失值的影响，VIMTS首先按时间间隔将IMTS处理为等间隔的特征.patch。然后，通过学习的跨通道依赖性来补充这些.patch。接下来，利用视觉MAE处理稀疏多通道数据的能力来进行.patch重建，并通过从集中上下文中生成精确预测的粗到细技术。此外，我们通过将视觉MAE适应IMTS数据来整合自监督学习，以改进IMTS建模。广泛的实验表明，VIMTS具有优越的性能和少样本能力，推动了视觉基础模型在更多一般时间序列任务中的应用。我们的代码可在以下网址获取。 

---
# CLUE: Neural Networks Calibration via Learning Uncertainty-Error alignment 

**Title (ZH)**: CLUE: 通过学习不确定性-误差对齐进行神经网络校准 
...
**Abstract (ZH)**: 可靠的不确定性估计对于在实际应用中部署神经网络至关重要。现有校准技术往往依赖于事后调整或粗粒度的分箱方法，但在可扩展性、可微性和跨域泛化能力方面仍有限制。在本文中，我们提出了CLUE（基于学习不确定性-误差对齐的校准方法），这是一种新型方法，在训练过程中显式地将预测的不确定性与观测到的误差对齐，其原理是校准良好的模型应该产生与实际损失相匹配的不确定性估计。CLUE采用了一种新的损失函数，该函数同时优化预测性能和校准，使用不确定性与损失的汇总统计数据作为代理。所提出的方法是完全可微的、领域无关的，并且与标准训练管道兼容。通过在视觉、回归和语言建模任务上的广泛实验，包括分布外和域迁移场景，我们证明了CLUE在相对于最新方法的校准质量方面表现优异，并在预测性能方面表现竞争力，而不会增加显著的计算开销。 

---
# Efficient Preimage Approximation for Neural Network Certification 

**Title (ZH)**: 神经网络认证的高效前image近似算法 
...
**Abstract (ZH)**: Physics Informed Neural Networks (PINNs)作为一种求解微分方程的强大计算工具正在兴起。然而，这些模型的应用尚处于初期阶段，需要更多的标准化才能获得更广泛的认可。通过这篇综述，我们提供了关于PINNs的各种架构、变体、应用领域、实际应用场景、挑战等方面的全面概述，试图填补现有文献中的空白，结合最近的进展和最新的研究进行详细分析。此外，我们讨论了PINNs实现中普遍存在的挑战，并提出了未来研究方向。综述的主要贡献可以总结为三个方面：详细概述PINNs的架构和变体，对PINNs在不同方程和应用领域的性能进行分析，最后详细讨论当前问题和未来的研究方向。 

---
# FAMA: The First Large-Scale Open-Science Speech Foundation Model for English and Italian 

**Title (ZH)**: FAMA: 首个大规模开放科学语音基础模型（英语和意大利语） 
...
**Abstract (ZH)**: 开放科学声学基础模型FAMA的发展：面向英语和意大利语的150K+小时开源声学数据训练模型 

---
# Self-orthogonalizing attractor neural networks emerging from the free energy principle 

**Title (ZH)**: 自由能原理涌现的自正交吸引子神经网络 
...
**Abstract (ZH)**: 自由能量原则应用于随机动力系统通用划分后的吸引子网络是如何涌现的：一种集体的多级贝叶斯主动推断过程 

---
# DeepMultiConnectome: Deep Multi-Task Prediction of Structural Connectomes Directly from Diffusion MRI Tractography 

**Title (ZH)**: DeepMultiConnectome: 直接从扩散MRI追踪图预测结构连接组的深度多任务学习 
...
**Abstract (ZH)**: 脑网络分析在诊断和监测阿尔茨海默病等神经退行性疾病中发挥着重要作用。现有的基于扩散张量成像(DTI)构建结构脑网络的方法通常依赖于专门的工具包，这些工具包存在固有的局限性：操作者的主观性、劳动密集型的工作流程以及捕获复杂拓扑特征和疾病特异性生物标志物的限制能力。为了克服这些挑战并推动计算神经影像学仪器的进步，提出了一种新的基于扩散的框架ConnectomeDiffuser，用于从DTI自动端到端构建脑网络。该提出的模型结合了三个关键组件：（1）模板网络，使用黎曼几何原理从3D DTI扫描中提取拓扑特征；（2）扩散模型，生成具有增强拓扑保真度的全面脑网络；（3）图卷积网络分类器，整合疾病特异性标记以提高诊断准确性。ConnectomeDiffuser通过捕获更广泛的结构连接性和与病理相关的信息，展示了优越的性能，使个体脑网络的分析更为敏感。在代表两种不同神经退行性疾病的数据库上的实验验证表明，与其他脑网络方法相比，其性能有了显著提升。该工作为神经退行性疾病背景下仪器的发展做出了贡献，为临床医生和研究人员提供了一种稳健且可泛化的测量框架，促进更准确的诊断、更深入的机制理解和更有效的神经退行性疾病如阿尔茨海默病的治疗监测。 

---
# PSBench: a large-scale benchmark for estimating the accuracy of protein complex structural models 

**Title (ZH)**: PSBench：用于评估蛋白质复合物结构模型准确性的大规模基准 
