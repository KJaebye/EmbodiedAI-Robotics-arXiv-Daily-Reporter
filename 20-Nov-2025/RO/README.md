...
**Title (ZH)**: In-N-On: 使用野外和任务中数据扩展自中心操作化 
...
**Abstract (ZH)**: 自躯干视角视频是学习操作策略的一个有价值且可扩展的数据来源。然而，由于数据显著异质性，大多数现有方法利用人类数据进行简单的预训练，未能充分利用其潜力。本文首先提供了一种可扩展的方法来收集和使用自躯干视角数据，并将人类数据分类为两类：野外数据和任务中数据，并进行了系统分析以利用这些数据。我们首先整理了一个名为PHSD的数据库，包含超过1000小时的多样性野外自躯干视角数据和超过20小时直接与目标操作任务对齐的任务中数据。这使得能够学习一个大型自躯干视角语言条件流匹配策略Human0。通过领域适应技术，Human0最小化了人类与类人者之间的差距。实验结果显示，Human0从扩展人类数据中获得了几种新颖特性，包括仅从人类数据遵循指令的能力、少样本学习以及使用任务中数据增强的鲁棒性。项目网站：这个https URL。 

---
# Real-time Point Cloud Data Transmission via L4S for 5G-Edge-Assisted Robotics 

**Title (ZH)**: 基于L4S的5G-边缘辅助 robotics实时点云数据传输 

**Authors**: Gerasimos Damigos, Achilleas Santi Seisa, Nikolaos Stathoulopoulos, Sara Sandberg, George Nikolakopoulos  

**Link**: [PDF](https://arxiv.org/pdf/2511.15677)  
...
**Abstract (ZH)**: 本文提出了一种利用率自适应技术和点云编码方法来确保低延迟和低丢包的实时激光雷达(LiDAR)数据传输新型框架，该框架旨在，但不仅限于需要通过互联网进行卸载处理的实时数据传输的机器人应用。具体而言，L4S启用的SCReAM v2传输框架被扩展以集成Draco几何压缩算法，根据感知到的信道容量和网络负载动态压缩高比特率的三维LiDAR数据。低延迟的三维LiDAR流传输系统旨在维持端到端延迟最小化的同时，约束编码误差以满足机器人应用的精度要求。通过在公共5G网络上进行的多公里城市环境中真实世界实验，展示了所提出方法的有效性，同时保留了低延迟和低丢包的要求，并通过实时卸载和评估三维SLAM算法来验证框架在实际应用场景中的性能。 
...
**Title (ZH)**: Optimus-Q：通过量子加密在适应性机器人中利用联邦学习以实现智能核电站运营 
...
**Abstract (ZH)**: 高级机器人在核电力植物中的集成为在高风险环境中增强安全、效率和环境监测提供了变革性机会。本文介绍了Optimus-Q机器人，这是一种先进的系统，能够自主监测空气质量并检测污染，同时利用自适应学习技术和安全的量子通信。装备有先进的红外传感器，Optimus-Q机器人不断传输实时环境数据，以预测有害气体排放，包括二氧化碳（CO₂）、一氧化碳（CO）和甲烷（CH₄）。通过联邦学习方法，该机器人与其他多个核电厂的系统协作以提高预测能力，而不牺牲数据隐私。此外，量子密钥分发（QKD）的应用确保了数据的安全传输，保护了敏感的操作信息。我们的方法结合了系统导航模式和机器学习算法，以优化指定区域的覆盖效率，从而优化污染监测过程。通过模拟和实际实验，我们证明Optimus-Q机器人在提高核设施的操作安全性和响应性方面是有效的。本研究强调了将机器人技术、机器学习和量子技术集成到危险环境中的监测系统中的潜在可能。 

---
# SRPO: Self-Referential Policy Optimization for Vision-Language-Action Models 

**Title (ZH)**: SRPO：自参照策略优化在视觉-语言-行动模型中的应用 
...
**Abstract (ZH)**: 基于视觉-语言-动作的强化学习：Self-Referential Policy Optimization框架在减轻专家演示依赖中的应用 

---
# UltraDP: Generalizable Carotid Ultrasound Scanning with Force-Aware Diffusion Policy 

**Title (ZH)**: UltraDP：带有力感知扩散策略的通用颈动脉超声扫描方法 

**Authors**: Ruoqu Chen, Xiangjie Yan, Kangchen Lv, Gao Huang, Zheng Li, Xiang Li  

**Link**: [PDF](https://arxiv.org/pdf/2511.15550)  
...
**Abstract (ZH)**: 自主水下车辆（AUVs）团队依靠声学通信进行协调，但该通信介质受到通信范围有限、多路径效应和低带宽的限制。为了应对声学通信相关的不确定性，可以实时学习通信环境。我们解决了机器人团队实时构建从一个位置到另一个位置通信成功率概率图的挑战。这是一个分散式分类问题——通信事件要么成功要么失败——其中AUVs共享一部分通信测量值以构建该图。本工作的主要贡献是一种严格 deriv 的数据共享策略，该策略选择了要在AUVs之间共享的测量值。我们使用来自弗吉尼亚技术学院690 AUV团队的实际声学通信数据验证了我们提出的数据共享策略，在水下环境中展示了其有效性。 

---
# Theoretical Closed-loop Stability Bounds for Dynamical System Coupled with Diffusion Policies 

**Title (ZH)**: 带有扩散策略耦合的动力系统闭环稳定性理论边界 
...
**Abstract (ZH)**: 基于扩散政策在随机扰动下执行任务中表现出的强大性能及其对多模态动作分布建模的能力，虽然其依赖于计算昂贵的逆向时间去噪过程进行动作推断，这使得它在需要快速决策的实时应用中难以使用。本文研究了在执行动作之前仅部分进行去噪过程的可能性，允许植物根据其动力学在计算机上的逆向时间去噪动力学进行的同时演化。在经典的扩散政策设置中，植物动力学通常较慢且两个动力学过程是解耦的。本文探讨了当植物动力学和去噪动力学耦合时使用扩散政策的闭环系统的稳定性理论界限。本文的贡献提供了一个更快速的模仿学习框架，并提供了一个指标来判断控制器的稳定性基于演示的方差。 

---
# Discovering Optimal Natural Gaits of Dissipative Systems via Virtual Energy Injection 

**Title (ZH)**: 通过虚拟能量注入发现耗散系统的最优自然步态 

**Authors**: Korbinian Griesbauer, Davide Calzolari, Maximilian Raff, C. David Remy, Alin Albu-Schäffer  

**Link**: [PDF](https://arxiv.org/pdf/2511.15513)  
...
**Abstract (ZH)**: 基于弹性元件利用自然动力学提高腿式机器人能量经济性的多阶段统一控制方法 

---
# RRT*former: Environment-Aware Sampling-Based Motion Planning using Transformer 

**Title (ZH)**: RRT*former: 基于变压器的环境感知采样运动规划 
...
**Abstract (ZH)**: 自主探索障碍丰富的空间需要确保效率的同时保证与障碍物碰撞的安全策略。本文研究了一种平台无关的强化学习框架，该框架结合了基于图神经网络的下一航点选择策略，并通过安全性过滤器确保移动安全。具体而言，神经网络利用 proximal 策略优化（PPO）算法通过强化学习进行训练，以最大化探索效率并最小化安全性过滤器的干预。当策略提出不可行的动作时，安全性过滤器将使用最近的可行替代方案进行覆盖，以保证系统的持续行为一致性。此外，本文引入了一个基于势场的奖励函数，该函数同时考虑了代理接近未探索区域的接近程度及其到达这些区域预期的信息增益。所提出的框架结合了基于强化学习的探索策略的适应性与通过显式安全机制提供的可靠性，这一特征在实现实现环境中的机器人平台部署学习策略时发挥了关键作用。广泛的仿真和实验室环境中的实验评估表明，该方法能够在复杂环境中实现高效且安全的探索。 

---
# C2F-Space: Coarse-to-Fine Space Grounding for Spatial Instructions using Vision-Language Models 

**Title (ZH)**: C2F-Space: 从粗到细的空间语义 grounding 用于基于视觉-语言模型的空间指令理解 

**Authors**: Nayoung Oh, Dohyun Kim, Junhyeong Bang, Rohan Paul, Daehyung Park  

**Link**: [PDF](https://arxiv.org/pdf/2511.15333)  
...
**Abstract (ZH)**: 空间定位指的是将自然语言指令中描述的一组空间参考进行 localization。传统方法往往无法处理复杂的推理——如距离、几何关系和物体间的关系——尽管视觉语言模型(Vision-Language Models, VLMs)具有强大的推理能力，但在产生细粒度输出区域方面仍然存在问题。为克服这些限制，我们提出了一种新颖的从粗到细的空间定位框架 C2F-Space，该框架包括：(i) 使用 VLM 估计一个近似的但具空间一致性区域；(ii) 通过超像素化将该区域细化以与局部环境对齐。为了粗略估计，我们设计了一个基于网格的视觉定位提示，并采用提出-验证策略，最大化 VLM 的空间理解能力，从而产生物理和语义有效的标准区域（例如椭圆）。在细化过程中，我们局部适应该区域以适应周围环境，而不会过度放宽以适应自由空间。我们构建了一个新的空间定位基准，并使用成功率和交并比与五种最先进的基准进行比较。我们的 C2F-Space 在所有基准中表现显著优异。我们的消融实验证实了两步过程中的每个模块的有效性及其组合框架的协同效果。最后，我们展示了 C2F-Space 在模拟机器人拾放任务中的适用性。 

---
# MSA - Technique for Stiffness Modeling of Manipulators with Complex and Hybrid Structures 

**Title (ZH)**: 基于MSA的复杂混合结构 manipulator 刚度建模技术 

**Authors**: Alexandr Klimchik, Anatol Pashkevich, Damien Chablat  

**Link**: [PDF](https://arxiv.org/pdf/2511.15294)  
...
**Abstract (ZH)**: 基于矩阵结构分析的复杂混合构型 manipulator 刚度建模系统方法 

---
# Optimizing Robot Positioning Against Placement Inaccuracies: A Study on the Fanuc CRX10iA/L 

**Title (ZH)**: 优化机器人定位精度以克服放置误差：Fanuc CRX10iA/L的研究 

**Authors**: Nicolas Gautier, Yves Guillermit, Mathieu Porez, David Lemoine, Damien Chablat  

**Link**: [PDF](https://arxiv.org/pdf/2511.15290)  
...
**Abstract (ZH)**: 这种研究提出了一种方法，用于确定fanuc CRX10iA/L协作机器人在预定工业任务相应轨迹的理想基座位置。该提出的办法使用了粒子群优化算法来探索搜索空间，以寻找执行轨迹的位置。随后使用$\alpha$-形状算法绘制可行性区域的边界，并通过Voronoi图计算内接圆的大小。这种方法旨在提供一种在可能存在机器人位置误差的情况下（例如，当系统由操作员部署时，机器人放置在移动基座上）的鲁棒性标准。所开发的方法使用了逆运动学模型来评估所有初始配置，然后使用雅可比矩阵移动机器人的末端执行器沿参考轨迹，并对其尝试进行评分。对于Fanuc CRX10iA/L机器人，逆运动学模型最多可以有16种解。这些解的计算并非易事，需要进行特定研究，而像MoveIt这样的规划工具不能完全考虑到这一点。此外，优化过程必须考虑关节限制、奇异性及工作空间限制，以确保轨迹执行的可行性与高效性。 
...
**Abstract (ZH)**: 动态环境中基于区域aware强化学习的路径规划方法 

---
# Look, Zoom, Understand: The Robotic Eyeball for Embodied Perception 

**Title (ZH)**: 看、聚焦、理解：类人眼球for体态感知 

**Authors**: Jiashu Yang, Yifan Han, Yucheng Xie, Ning Guo, Wenzhao Lian  

**Link**: [PDF](https://arxiv.org/pdf/2511.15279)  
...
**Abstract (ZH)**: 具有躯体化的AI感知系统中的视觉感知应该是主动的：目标是在像素和空间预算的约束下主动获取更具信息量的数据，而非被动处理静态图像。现有的视觉模型和固定的RGB-D相机系统无法兼顾广泛的场景覆盖与精细的细节获取，严重限制了其在开放环境下机器人应用中的效果。为解决这一问题，我们提出EyeVLA，一种能够基于指令采取主动行为的机器人眼球，用于实现对精细目标物体的清晰观察和广泛空间范围内的详细信息获取。EyeVLA将行动行为离散化为行动令牌，并将其与具备强大的开放世界理解能力的视觉语言模型（VLMs）集成，能够在单个自回归序列中联合建模视觉、语言和行动。通过使用2D边界框坐标引导推理链，并应用强化学习来优化视角选择策略，我们仅使用少量的现实世界数据，将VLM的开放世界场景理解能力转移到视觉语言行动（VLA）策略中。实验表明，我们的系统能够高效地在真实环境中执行指令场景，并通过指令驱动的旋转和缩放动作主动获取更准确的视觉信息，从而实现强大的环境感知能力。EyeVLA引入了一种新型的机器人视觉系统，利用详尽且空间丰富的大规模实体数据，并主动获取高度信息丰富的视觉观察，以支持下游的实体任务。 

---
# Behavior Trees vs Executable Ontologies: a Comparative Analysis of Robot Control Paradigms 

**Title (ZH)**: 行为树与可执行本体：机器人控制范式的比较分析 
...
**Abstract (ZH)**: 本文比较了两种不同的机器人行为建模方法：命令式行为树(BT)和声明式可执行本体(EO)，并借助boldsea框架实现。行为树通过控制流以层次结构形式组织行为，而可执行本体则通过数据流规则以基于事件的时间语义图形式表示领域。我们通过从根本上不同的架构证明，可执行本体在实现与行为树相当的反应性和模块化方面是可行的：使用事件驱动的状态传播替代基于轮询的.tick.执行。我们提出，可执行本体提供了另一种框架，从过程化编程转向基于语义领域的建模，以解决传统机器人控制中的语义-过程差距。可执行本体支持运行时模型修改、完整的时态可追溯性和数据、逻辑和界面的一致表示——这些特性在行为树中难以实现或有时是不可能实现的，尽管行为树在已建立的可预测场景中表现出色。这种比较在实际的移动 manipulation 任务中得到了体现。该比较突显了每种方法在动态、演变型机器人系统中的操作优势。 

---
# Symmetry-Breaking in Multi-Agent Navigation: Winding Number-Aware MPC with a Learned Topological Strategy 

**Title (ZH)**: 多智能体导航中的对称性破缺：基于学习拓扑策略的顾及布旋数的模型预测控制 

**Authors**: Tomoki Nakao, Kazumi Kasaura, Tadashi Kozuno  

**Link**: [PDF](https://arxiv.org/pdf/2511.15239)  
...
**Abstract (ZH)**: 我们提出一种新的分层导航方法，以解决分布式多智能体导航中由对称性引起的死锁基本挑战。通过引入一种使用拓扑不变量缠绕数来量化合作对称性打破策略的方法，并通过强化学习学习这些策略本身来应对这一问题。该方法包含一个分层策略，由基于学习的规划器和基于模型的控制器组成。规划器通过强化学习学习生成两种类型的参数来控制器：一种是用缠绕数表示的拓扑合作策略，另一种是一组动态权重，用于确定在多个智能体同时交叉的密集场景中优先处理哪种交互。控制器然后根据规划器提供的策略和权重生成无碰撞且高效的运动。该分层结构将基于学习的方法的灵活决策能力与基于模型的方法的可靠性相结合。仿真和真实世界机器人实验表明，与现有基线方法相比，我们的方法在密集环境中表现出色，能够高效地避免碰撞和死锁，同时实现更好的导航性能。实验代码可在以下链接获得：this https URL。 

---
# Modelling and Model-Checking a ROS2 Multi-Robot System using Timed Rebeca 

**Title (ZH)**: 基于Timed Rebeca对ROS2多机器人系统的建模与模型检查 

**Authors**: Hiep Hong Trinh, Marjan Sirjani, Federico Ciccozzi, Abu Naser Masud, Mikael Sjödin  

**Link**: [PDF](https://arxiv.org/pdf/2511.15227)  
...
**Abstract (ZH)**: 基于模型的开发能够加速原型制作、提前验证设计意图。对于具有复杂异步交互和并发性的多代理系统，形式验证，尤其是模型检测，提供了一种自动验证所需属性的机制。Timed Rebeca是一种基于演员的建模语言，支持反应性、并发性和时间语义，并配有一个模型检测编译器。这些能力使用户能够使用Timed Rebeca正确地建模ROS2节点拓扑、重复的物理信号、运动基本动作以及其他定时和可转换定时的行为。多机器人系统建模与验证的最大挑战在于抽象复杂信息、缩小离散模型与连续系统之间的差距、压缩状态空间同时保持模型的准确性。我们为不同类型的信息开发了不同的离散化策略，确定了抽象的“足够”阈值，并应用高效的优化技术来加速计算。通过本项工作，我们展示了如何使用模型设计和验证多机器人系统、如何离散建模连续系统以高效进行模型检测以及模型与实现之间的往返工程流程。发布的Rebeca和ROS2代码可以作为建模多个自主机器人系统的基础。 

---
# A Class of Dual-Frame Passively-Tilting Fully-Actuated Hexacopter 

**Title (ZH)**: 一类双框架被动倾斜全驱动六旋翼无人机 

**Authors**: Jiajun Liu, Yimin Zhu, Xiaorui Liu, Mingye Cao, Mingchao Li, Lixian Zhang  

**Link**: [PDF](https://arxiv.org/pdf/2511.15225)  
...
**Abstract (ZH)**: 本文提出了一种新型全驱动六旋翼无人机。它配备双框架被动倾斜结构，通过最少的驱动器实现了独立的平移运动和姿态控制。相比之前的全驱动无人机，它消除了内部力的抵消，使得在相同载荷条件下具有更高的飞行效率和续航能力。基于全驱动六旋翼无人机的动力学模型，设计了一种全驱动控制器以实现高效稳定的控制。最后，通过仿真验证了全驱动六旋翼无人机的优越全驱动运动能力和所提出控制策略的有效性。 

---
# VIRAL: Visual Sim-to-Real at Scale for Humanoid Loco-Manipulation 

**Title (ZH)**: VIRAL: 视觉模拟到现实的大规模人体仿躯干操作 
...
**Link**: [PDF](https://arxiv.org/pdf/2511.15200)  
...
**Abstract (ZH)**: 视觉导向的模拟到现实框架VIRAL：学习类人机器人的自主 locomotion+manipulation技能并零样本部署到真实硬件 

---
# Eq.Bot: Enhance Robotic Manipulation Learning via Group Equivariant Canonicalization 

**Title (ZH)**: Eq.Bot: 通过群体等变标准化增强机器人操作学习 
...
**Abstract (ZH)**: 基于SE(2)群equivariant理论的通用canonical化框架：为机器人操作学习提供空间equivariance 
...
**Authors**: Angshu Adhya, Cindy Yang, Emily Wu, Rishad Hasan, Abhishek Narula, Patrícia Alves-Oliveira  

**Link**: [PDF](https://arxiv.org/pdf/2511.15105)  
...
**Abstract (ZH)**: 本工作中我们提出了AURA，一种协同人类艺术家绘画的框架。我们开发了一种机器人手臂，能够与人类艺术家协作绘画。该机器人通过EmotiBit传感器感知艺术家的心跳，从而提供作画者的唤醒水平。根据检测到的心跳，机器人决定增加与艺术家工作区的距离或退缩。如果检测到较高的心跳，这与人类艺术家的较高唤醒水平相关联，机器人将远离画布的该区域。如果检测到艺术家的心率为中性，表明人类艺术家的基础状态，机器人将在整个画布上继续其绘画动作。我们还展示了并提出了使用自然语言和物理接触的机器人-艺术家交互方式。本工作结合了人类艺术家的生理数据，以促进流畅的艺术互动。 

---
# Lie Group Control Architectures for UAVs: a Comparison of SE2(3)-Based Approaches in Simulation and Hardware 

**Title (ZH)**: 基于SE2(3)的方法在 simulink 和硬件平台上的无人机运动控制架构比较 

**Authors**: Dimitria Silveria, Kleber Cabral, Peter Jardine, Sidney Givigi  

**Link**: [PDF](https://arxiv.org/pdf/2511.15023)  
...
**Abstract (ZH)**: 基于示教的Learning from Demonstration (LfD) 已经展示了为各种领域提供机器人基本运动技能的能力。LfD 研究的不同分支（例如，学习动力学系统和运动原型）通常可以被分类为“时间依赖型”或“时间独立型”系统。每种方法都有其基本的优势和劣势——时间独立方法无法学习重叠轨迹，而时间依赖性则可能在扰动下导致不良行为。本文引入了Cluster Alignment for Learned Motions (CALM)，这是一种基于与所演示运动的代表“均值”轨迹对齐的LfD框架，而不是纯粹的时间依赖或状态依赖。我们讨论了CALM的收敛特性，介绍了一种能够处理扰动下对齐变化的对齐技术，并利用演示聚类生成多模态行为。我们展示了CALM如何在二维数据集上缓解时间依赖和时间独立技术的缺陷，并在三个不同领域上的7自由度机器人任务中实现了我们的系统。 
...
**Abstract (ZH)**: 随着更多自动驾驶车辆在公共道路上运行，了解自动驾驶车辆的实际行为对于分析交通安全性、制定政策以及公众接受度至关重要。本文提出了一种SVBRD-LLM框架，该框架通过零样本提示工程自动发现、验证并应用于来自真实交通视频的行为规则。该框架使用YOLOv8和ByteTrack提取车辆轨迹，计算运动特征，并使用GPT-5零样本提示比较自动驾驶车辆和人工驾驶车辆，生成35个结构化行为规则假设。这些规则在验证集上进行测试，基于失败案例迭代精炼以过滤掉虚假相关性，并编译成高置信度规则库。该框架在独立测试集上针对车道变更预测、速度变更预测以及自动驾驶车辆识别任务进行了评估。在超过1500小时的真实交通视频实验中，框架在自动驾驶车辆识别任务上的准确率为90.0%，F1分为93.3%。发现的规则明显揭示了自动驾驶车辆在速度控制平滑性、车道变更保守性以及加速度稳定性方面的独特特征，每个规则都附有语义描述、适用场景和验证置信度。 

---
# A visual study of ICP variants for Lidar Odometry 

**Title (ZH)**: ICP变体的视觉研究在激光雷达里程计中的应用 

**Authors**: Sebastian Dingler, Hannes Burrichter  

**Link**: [PDF](https://arxiv.org/pdf/2511.14919)  
...
**Abstract (ZH)**: 基于激光雷达传感器的里程计是一种先进的方法，用于估算移动车辆的自车姿态。许多激光雷达里程计的实现使用ICP算法的变体。现实世界中的动态物体、非重叠区域和传感器噪声降低了ICP的准确性。我们在此基础上提出了一种方法，通过将ICP的多维目标函数可视化为二维图来使这些效应变得可见。我们使用该方法研究了不同ICP变体在激光雷达里程计中的应用。此外，我们提出了一种新的方法来筛选动态物体并解决自车盲区问题。 

---
# Z-Merge: Multi-Agent Reinforcement Learning for On-Ramp Merging with Zone-Specific V2X Traffic Information 

**Title (ZH)**: Z-合并：基于区域特定V2X交通信息的多agent强化学习匝道汇入控制 

**Authors**: Yassine Ibork, Myounggyu Won, Lokesh Das  

**Link**: [PDF](https://arxiv.org/pdf/2511.14910)  
...
**Abstract (ZH)**: 基于V2X辅助的多代理强化学习匝道并线框架 
...
**Abstract (ZH)**: 通过链式思考（Chain-of-Thought）使视觉-语言-动作（VLA）模型在“思而后行”：一种克服端到端机器人策略的数据饥渴性质的有前途路径及DeepThinkVLA的解决策略 

---
# MambaIO: Global-Coordinate Inertial Odometry for Pedestrians via Multi-Scale Frequency-Decoupled Modeling 

**Title (ZH)**: MambaIO：行人多尺度频域解耦惯性里程计全局坐标惯性里程计 

**Authors**: Shanshan Zhang  

**Link**: [PDF](https://arxiv.org/pdf/2511.15645)  
...
**Abstract (ZH)**: 基于惯量的里程计 (Inertial Odometry, IO) 能够仅通过惯性测量单元 (IMU) 获取的加速度和角速度测量值实现实时定位，使其成为消费级应用中定位的有前途的解决方案。传统上，IO 中的 IMU 测量值在两种坐标系框架下处理：体坐标框架和全局坐标框架，后者被广泛采用。然而，最近在无人机场景中的研究表明，体坐标框架可以显著提高定位精度，促使重新评估全局框架在行人IO中的适用性。为解决这一问题，本文系统地通过理论分析、定性检验和定量实验评估了全局坐标框架在行人IO中的有效性。在此基础上，我们进一步提出 MambaIO，该方法通过拉普拉斯金字塔将 IMU 测量值分解为高频和低频分量。低频分量由 Mamba 架构处理以提取隐含的上下文运动线索，而高频分量则由卷积结构处理以捕获精细的局部运动细节。在多个公开数据集上的实验表明，MambaIO 显著降低了定位误差并达到了最先进的 (SOTA) 性能。据我们所知，这是首次将 Mamba 架构应用于惯量里程计任务。 

---
# PCARNN-DCBF: Minimal-Intervention Geofence Enforcement for Ground Vehicles 

**Title (ZH)**: PCARNN-DCBF: 基于最小干预的地面上车辆地理围栏执行方法 

**Authors**: Yinan Yu, Samuel Scheidegger  

**Link**: [PDF](https://arxiv.org/pdf/2511.15522)  
...
**Abstract (ZH)**: 开发可靠的无人机导航系统需要稳健的空中目标检测器，能够区分训练中见到的对象和前所未见的对象。现有方法通常专注于闭集检测，实现了对域内目标的高置信度识别，但通常不处理开放集检测，后者需要同时处理已知对象和不属分布对象。现有的开放集方法通常依赖单一的不确定性评分和阈值划分，限制了灵活性，并且经常将不属分布对象与背景杂波混淆。相比之下，我们提出了一种轻量级、模型无关的后处理框架，在保留基础检测器性能的同时明确地将背景与未知对象分离。我们的方法将开放集检测扩展到实时三分类，即对已知目标、不属分布对象和背景进行分类。为此，我们采用了一种融合方案，利用一个紧凑的多层感知机（MLP）聚合多个置信估计和单个检测特征。将不同类型的logit变体整合到MLP中，在二分类和三分类中保持了性能的提升，且不牺牲吞吐量。广泛的消融实验和对比实验表明，与基于阈值的基线方法相比，我们的方法在二分类中的平均AUROC提升2.7%，同时保留或提高了开放集mAP。此外，我们的研究首次实现了稳健的三分类，这是安全无人机导航所需的关键能力，其中不属分布对象必须主动避免，背景区域则需安全忽略。对比分析显示，我们的方法在不同数据集上的AUROC上超越了竞争性技术，同时在闭集mAP上提高了高达9个点，即18%的相对增益。 

---
# Nonholonomic Robot Parking by Feedback -- Part I: Modular Strict CLF Designs 

**Title (ZH)**: 非完整机器人停车控制——第一部分：模块化严格CLF设计 

**Authors**: Velimir Todorovski, Kwang Hak Kim, Alessandro Astolfi, Miroslav Krstic  

**Link**: [PDF](https://arxiv.org/pdf/2511.15119)  
...
**Abstract (ZH)**: 自1995年以来，机器人文献中已知，非holonomic单轨车在极坐标系下可以通过光滑反馈全局渐近稳定化。我们引入了一种模块化设计框架，选择前向速度以解耦径向坐标，使得转向子系统可以独立稳定化。在该结构内部，我们利用.passivity.、.backstepping.和.积分前馈.发展了一组反馈律。每一律都有严格的控制李亚普诺夫函数，包括强制角向约束的屏障变体。这些严格的CLFs提供了构造性的类KL收敛估计，并允许在目标平衡点处进行特征值分配。该框架扩展了先前的模块化和非模块化方法，为后续论文中的反最优和自适应重新设计奠定了基础。 

---
# Learning Human-Like RL Agents Through Trajectory Optimization With Action Quantization 

**Title (ZH)**: 通过动作量化进行轨迹优化学习人类般的RL代理 
...
**Abstract (ZH)**: 类人代理在强化学习中的实现：轨迹优化与宏观动作量化 
...
**Title (ZH)**: EGSA-PT：基于边缘引导的空间注意力机制与分阶段训练在单目透明物体深度估计与分割中的应用 
...
**Abstract (ZH)**: 透明对象感知仍然是计算机视觉研究中的一个主要挑战，因为透明性既干扰深度估计又干扰语义分割。最近的工作探索了多任务学习框架以提高鲁棒性，但跨任务的负交互作用往往会阻碍性能。在本文中，我们提出了边界引导空间注意力（EGSA）机制，通过将边界信息纳入语义和几何特征的融合中，旨在减轻破坏性交互作用。在Syn-TODD和ClearPose基准测试中，EGSA始终在当前最先进的方法（MODEST）上提高了深度精度，同时保持了竞争力的分割性能，最大的改进出现在透明区域。除了我们的融合设计，我们还贡献了一种多模态渐进训练策略，在这种策略中，学习从RGB图像导出的边缘过渡到由预测深度图像导出的边缘。这种方法使得系统可以从RGB图像中丰富的纹理中启动学习，然后切换到深度图中的更相关的几何内容，同时消除了训练时需要地面真值深度的需求。这两方面的贡献共同强调了边界引导融合作为一种稳健的方法，能够改善透明对象感知。 
...
**Abstract (ZH)**: 自动驾驶系统中对抗样本的风险评估：对多种驾驶代理的攻击策略评价 
