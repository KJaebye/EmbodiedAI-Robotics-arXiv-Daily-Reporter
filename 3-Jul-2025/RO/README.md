...
**Abstract (ZH)**: 生物多智能体机器人系统（BioMARS）：基于大规模语言模型和视觉语言模型的智能生物学实验自主设计与执行平台 
...
**Abstract (ZH)**: 利用大型语言模型进行少样本代码生成以自动在CARLA模拟器中合成多样化和安全关键的驾驶场景 
