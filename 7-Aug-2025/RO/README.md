...
**Title (ZH)**: DRIVE: 动态规则推理与约束感知自主驾驶的验证评估 
...
**Abstract (ZH)**: 基于约束保持数据生成的大规模机器人操作演示数据驱动方法 
