...
**Title (ZH)**: DexCanvas: 联通人类示范与机器人灵巧操作学习 

**Authors**: Xinyue Xu, Jieqiang Sun, Jing, Siyuan Chen, Lanjie Ma, Ke Sun, Bin Zhao, Jianbo Yuan, Yiwen Lu  

**Link**: [PDF](https://arxiv.org/pdf/2510.15786)  
...
**Abstract (ZH)**: 本工作在自主机器人探索的环境规模和探索效率方面推动了基于学习方法的边界。我们提出了HEADER，一种基于注意力的强化学习方法，结合分层图，以提高大型环境下的探索效率。 HEADER 在构建机器人信念/地图的分层表示方面遵循现有的传统方法，但进一步设计了一种基于社区的新颖算法来构建和更新全局图，该算法保持完全增量、形状自适应，并具有线性复杂度。基于基于注意力的网络，我们的规划者在局部范围内精细地考虑附近的信念，同时在全局范围内粗略地利用远端信息，从而实现多尺度空间依赖性的下一步最优视点决策。除了新颖的地图表示，我们引入了一种无需参数的特权奖励，该奖励显着提高了模型性能并产生接近最优的探索行为，通过避免由手工构建的奖励塑形造成的训练目标偏差。在模拟的具有挑战性、大型环境探索场景中，HEADER 在可扩展性方面优于大多数现有学习和非学习方法，同时相对于最先进的基线方法在探索效率上实现了显著改进（高达20%）。我们还在硬件上部署了HEADER，并在复杂的大型实际场景中进行了验证，包括一个300m×230m的校园环境。 
...
**Abstract (ZH)**: 基于Variable Stiffness Link的长距空中 manipulation系统研究 

---
# Educational SoftHand-A: Building an Anthropomorphic Hand with Soft Synergies using LEGO MINDSTORMS 

**Title (ZH)**: 教育软手-A：基于LEGO MINDSTORMS构建类人软手协同系统 

**Authors**: Jared K. Lepora, Haoran Li, Efi Psomopoulou, Nathan F. Lepora  

**Link**: [PDF](https://arxiv.org/pdf/2510.15638)  
...
**Abstract (ZH)**: 基于LEGO MINDSTORMS的教育软手A：一种基于比萨/意大利理工软手的腱驱动高欠驱动仿人机器人手及其设计与应用 

---
# Adaptive Legged Locomotion via Online Learning for Model Predictive Control 

**Title (ZH)**: 基于在线学习的自适应腿式运动模型预测控制 

**Authors**: Hongyu Zhou, Xiaoyu Zhang, Vasileios Tzoumas  

**Link**: [PDF](https://arxiv.org/pdf/2510.15626)  
...
**Abstract (ZH)**: 机械系统的名义性能往往受到未知干扰的影响。两自由度控制结构可以从干扰抑制中解耦名义性能。然而，当干扰动力学未知时，完美的干扰抑制是不可能实现的。在本文中，我们揭示了在追踪速度和追踪不确定性约束下的干扰估计固有的权衡。然后，我们提出了两种新的方法来增强干扰估计：基于交互式多种模型扩展卡尔曼滤波器的干扰观测器和基于多核正态逼近滤波器的干扰观测器。实验结果表明，在时间varying交互力场景下，所提出的方法分别将髋关节误差的跟踪精度提高了36.3%和16.2%，膝关节误差的跟踪精度提高了46.3%和24.4%，相比基于扩展卡尔曼滤波器的干扰观测器，证明了所提出方法的优越性。 
...
**Title (ZH)**: 完美预测还是充足建议？规划自主驾驶中何者更为重要 

**Authors**: Aron Distelzweig, Faris Janjoš, Oliver Scheel, Sirish Reddy Varra, Raghu Rajan, Joschka Boedecker  

**Link**: [PDF](https://arxiv.org/pdf/2510.15505)  
...
**Abstract (ZH)**: 传统上，自主驾驶中的预测和规划被视为分离的、顺序的模块。近年来，人们越来越倾向于将这些组件通过集成预测和规划（IPP）进行更紧密的集成，目的是实现更具信息量和适应性的决策。然而，目前仍不清楚这种集成在多大程度上改善了规划性能。在本文中，我们研究了IPP方法中预测的作用，采用了广泛采用的Val14基准测试，该基准涵盖了具有相对较低交互复杂性的常见驾驶场景，以及包括高度交互和分布外驾驶情况的interPlan基准测试。我们的分析表明，即使拥有完美的未来预测信息也不一定能得到更好的规划结果，表明当前的IPP方法往往未能充分利用未来行为信息。相反，我们侧重于高质量提案的生成，主要利用预测进行碰撞检查。我们发现，许多基于模仿学习的规划器难以生成逼真且合理的提案，其表现甚至不如简单的车道跟随方法PDM。受此观察的启发，我们基于PDM构建了一个改进的提案生成方法，着重于生成多样化但逼真且高质量的提案。这种以提案为中心的方法在分布外和高度交互的设置中显著优于现有方法，特别是在这些设置中，它设定了新的最佳成果。 

---
# VDRive: Leveraging Reinforced VLA and Diffusion Policy for End-to-end Autonomous Driving 

**Title (ZH)**: VDRive: 利用强化VLAD和扩散策略实现端到端自动驾驶 

**Authors**: Ziang Guo, Zufeng Zhang  

**Link**: [PDF](https://arxiv.org/pdf/2510.15446)  
...
**Abstract (ZH)**: 自动化去鸡肩骨要求通过部分遮挡、可变形的多材料关节进行精确的6-自由度切割，由于与骨骼接触会带来严重的健康和安全风险。我们的工作在系统级和算法级上都做出了贡献，以训练并部署一种能动态调整预定轨迹的反应式力反馈切割策略，并且能够实现6-自由度刀具控制以通过狭窄的关节间隙并避免接触骨骼。首先，我们引入了一个开源的自定义多材料切割模拟器，该模拟器模型了耦合、断裂和切割力，并支持强化学习，从而实现高效训练和快速原型制作。其次，我们设计了一个可重复使用的物理试验台来模拟鸡肩：两个具有可控姿态的刚性“骨骼”球体嵌入较软的块体中，从而保持了目标问题的主要多材料特性，同时也能够进行严格的重复评估。第三，我们训练并部署了一个离散力观测残差RL策略，结合领域随机化，实现了稳健的无监督模拟到现实世界的转移，并展示了第一个能够去真鸡肩骨的从学习策略。我们在模拟器、物理试验台和真实鸡肩上的实验表明，我们的学习策略能可靠地通过关节间隙并减少不必要的骨骼/软骨接触，相比于现有的开环切割基线，在成功率和骨骼规避方面提升了高达4倍的效果。我们的结果还展示了力反馈在安全有效的多材料切割中的必要性。项目网站：🔗。 

---
# GaussGym: An open-source real-to-sim framework for learning locomotion from pixels 

**Title (ZH)**: GaussGym: 一个开源的从像素学习行走的实时到模拟框架 
...
**Abstract (ZH)**: 自主 underwater 机器人必须在强流、有限的声学带宽以及持续的传感要求下运作，传统群体优化方法在此情况下不可靠。本文提出了一种新颖的受自然界启发的群体优化算法 NOAH，该算法结合了流感知漂移、不可逆的持续传感节点锚定机制以及基于群体的通信。NOAH 通过借鉴浮泥虫的行为，解决了现有群体算法的关键限制，提供了水动力感知、不可逆锚定机制和基于群体的通信能力，这些对于水下探索任务至关重要。该算法为可扩展和节能的水下群体机器人奠定了基础，并经过了验证性分析。验证研究表明，在永久锚定场景中成功率高达 86%，并通过流场下的实验证明了一种统一的水动力约束和不可逆锚定行为的表述。 

---
# Adaptive Cost-Map-based Path Planning in Partially Unknown Environments with Movable Obstacles 

**Title (ZH)**: 基于可移动障碍物的部分未知环境自适应成本图路径规划 
...
**Link**: [PDF](https://arxiv.org/pdf/2510.15336)  
...
**Abstract (ZH)**: 可靠的灾区响应和其它非结构化室内导航需要机器人不仅避障，还能识别可推移的障碍物。一种适应性LiDAR和里程计路径规划框架嵌入ROS2 Nav2堆栈中以实现这一能力。一个新的可移动障碍物层将所有先前静态地图中缺失的LiDAR回波标记为临时可移动，并分配较低的穿越成本。一个配套的慢速位姿进展检查器监控指令速度与实际速度的比例；当机器人减速明显时，局部成本从轻量级提高到重度，停滞时提高到致命级，促使全局规划器回退并重新规划路径。针对Scout Mini的Gazebo评估表明，在孤立物体和杂乱走廊场景下，与无层基线相比，目标可达率更高，死锁更少，穿越时间大致相当。由于该方法仅依赖于平面扫描和CPU级计算，它适用于资源受限的搜索救援机器人，并可最小化工程量地集成到异构平台上。总体而言，结果表明，交互意识成本图是导航到潜在可移动障碍物区域的一种轻量级且原生的ROS2扩展。完整的实现将于公开源代码发布于https://costmapthis http URL。 

---
# ASBI: Leveraging Informative Real-World Data for Active Black-Box Simulator Tuning 

**Title (ZH)**: ASBI：利用信息丰富的实际数据进行活性黑盒模拟器调优 
...
**Abstract (ZH)**: 场景图增强机器人3D建图能力通过理解不同空间元素之间的关系，并将其扩展至层次结构，提高同时定位与建图的准确性和效率。然而，在分割空间特征时，一致地识别房间由于视角变化和传感器有限视场（FOV）而变得具有挑战性。为此，我们提出了一种考虑机器人与周围环境交互的可通行性感知房间分割方法，该方法确保了可通行性信息的持续一致性，从而提高姿态图优化的语义连贯性和计算效率。性能改进通过同一空间沿相同路径多次穿越的数据集中的相同房间检测频率和优化时间消耗得以验证。 

---
# A Generalized Sylvester-Fermat-Torricelli problem with application in disaster relief operations by UAVs 

**Title (ZH)**: 广义赛尔维斯特-费马-托里切利问题及其在无人机救灾中的应用 

**Authors**: Sina Kazemdehbashi, Yanchao Liu, Boris S. Mordukhovich  

**Link**: [PDF](https://arxiv.org/pdf/2510.15229)  
...
**Abstract (ZH)**: 自然和人为灾害会导致严重的破坏并夺走数千条生命。因此，对于救援团队而言，开发高效的灾害响应和管理方法是一项关键任务。有效响应的核心之一是快速收集受灾区域、损失和受害者的信息。更多信息意味着更好的协调、更快的救援行动，最终拯救更多生命。然而，在地震等灾害中，通信基础设施经常部分或完全被破坏，使得受害者发送求救信号和救援队伍及时定位并提供援助变得极其困难。无人驾驶航空器（UAV）在这种情况下成为了有价值的工具。特别是在这种场景中，可以派遣一系列UAV从移动站出发到达受灾区域以促进数据收集并建立临时通信网络。然而，实际部署UAV面临着一系列挑战，其中最严峻的是恶劣天气条件，特别是风力。为此，我们开发了一个新颖的数学框架，以确定移动UAV站的最佳位置，同时明确考虑了UAV的异质性和风的影响。具体来说，我们将塞瓦尔德问题推广为引入塞瓦尔德-费马-托里埃利（SFT）问题，该问题在一个统一的框架中捕捉了风的影响、UAV的异质性和往返运动等复杂因素。所提出框架通过考虑实际因素，如风和UAV异质性，提高了基于UAV的灾害响应规划的实用性。实验结果表明，它可以将无效操作时间减少多达84%，使灾害后的任务显著更高效和更有成效。 

---
# PolyFly: Polytopic Optimal Planning for Collision-Free Cable-Suspended Aerial Payload Transportation 

**Title (ZH)**: PolyFly: 多面体最优规划实现无碰撞的悬索空中运载货物传输 

**Authors**: Mrunal Sarvaiya, Guanrui Li, Giuseppe Loianno  

**Link**: [PDF](https://arxiv.org/pdf/2510.15226)  
...
**Abstract (ZH)**: 悬索架设的飞行机器人已发展成为灾害响应和救援操作中多功能的平台。为最大限度地提高这些系统的性能，机器人需要在诸如密集森林和结构不安全建筑等严苛受限环境中进行高效的飞行，同时最小化飞行时间和避免障碍。现有方法在几何上过度近似无人机和障碍物，导致保守的操控方式和增加的飞行时间。我们通过提出PolyFly，一个全局最优规划器，来消除这些限制。PolyFly考虑了非保守的表示方法，通过建模环境中的每个物理组件以及无人机（四旋翼机、悬索和载荷）为独立的多面体来进行空中运输。为进一步提高建模准确性，我们通过构建姿态感知的多面体来融入物理组件的姿态。通过二重性理论将多面体约束转化为平滑可微约束，从而高效地求解最优控制问题。我们在八个迷宫样环境中与现有最佳方法进行了对比，结果显示PolyFly在每种情景下均产生更快的轨迹。我们还在实际的悬索四旋翼机上进行了实验验证，证明了该方法的实用可靠性和准确性。 
...
**Abstract (ZH)**: 复杂动态环境中腿式机器人自主导航依赖于 robust 同时定位与建图 (SLAM) 系统，以准确地映射环境并定位机器人，确保安全高效的运行。为此，我们提出了一种 robust LiDAR-视觉-惯性-运动学里程计系统，该系统结合了如相机、LiDAR、惯性测量单元（IMU）和关节编码器等多种传感器信息，用于基于视觉和LiDAR的里程计估计。我们的系统采用基于融合的姿态估计方法，根据不同传感器测量数据的可用性运行优化基于视觉-惯性-运动学里程计（VIKO）和基于滤波器的LiDAR-惯性-运动学里程计（LIKO）算法。在VIKO中，我们使用足部预积分技术和滑窗优化中的鲁棒LiDAR-视觉深度一致性（基于超像素簇）。在LIKO中，我们结合了足部运动学，并在错误状态迭代卡尔曼滤波器（ESIKF）中使用点到面残差。与其他基于传感器融合的SLAM算法相比，我们的方法在公共数据集和长期数据集上均表现出 robust 性能。 

---
# Lagrange-Poincaré-Kepler Equations of Disturbed Space-Manipulator Systems in Orbit 

**Title (ZH)**: 轨道上受扰空间操作系统的拉格朗日-庞加莱-开普勒方程 

**Authors**: Borna Monazzah Moghaddam, Robin Chhabra  

**Link**: [PDF](https://arxiv.org/pdf/2510.15199)  
...
**Abstract (ZH)**: 这篇文章提出了Lagrange-Poincare-Kepler方程（LPKE）以扩展拉格朗日-庞加莱方程（LPE），用于建模在非惯性轨道参考框架内运行的航天器- manipulator 系统的动力学。在此前对车辆- manipulator 系统的LPE表述的基础上，提出的框架结合了航天器姿态动力学、轨道运动和 manipulator 运动学，使用指数关节参数化。通过在主丛上应用拉格朗日-达朗贝尔原理，我们推导出新颖的闭式结构矩阵，明确地捕捉轨道扰动及其与 manipulator 系统的动力学耦合效应。LPKE框架系统地包含了外部施加的、破坏对称性的力偶矩，使其能够直接集成到硬件在环仿真和基于模型的控制架构中，以实现轨道环境中的自主机器人操作。为了阐明所提出模型的有效性和数值优势，我们进行了一项仿真研究，分析了轨道效应对附着在航天器上的7自由度 manipulator 的影响。 

---
# RM-RL: Role-Model Reinforcement Learning for Precise Robot Manipulation 

**Title (ZH)**: RM-RL: 基于角色模型的强化学习精确机器人操作 

**Authors**: Xiangyu Chen, Chuhao Zhou, Yuxi Liu, Jianfei Yang  

**Link**: [PDF](https://arxiv.org/pdf/2510.15189)  
...
**Abstract (ZH)**: 精确机器人操作对于化学和生物实验等精细应用至关重要，即使是非常小的错误（如试剂溢出）也会使整个任务失效。现有的方法通常依赖于预先收集的专家演示，并通过模仿学习（IL）或离线强化学习（RL）训练策略。然而，为精确任务获取高质量的演示是困难且耗时的，而离线RL通常会遭受分布偏移和低数据效率的问题。我们提出了一个角色模型强化学习（RM-RL）框架，将在线和离线训练统一于现实环境。关键思想是角色模型策略，该策略可以自动为在线训练数据生成标签，从而消除对人类演示的需要。RM-RL将策略学习重新表述为监督训练，减少了分布不匹配带来的不稳定性，提高了效率。混合训练方案进一步利用在线角色模型数据进行离线重用，通过重复采样提高数据效率。大量实验证明，RM-RL比现有RL方法更快、更稳定地收敛，实现在现实世界操作中的显著提升：平移精度提高53%，旋转精度提高20%。最后，我们展示了精确将细胞板放置到架子上的复杂任务的成功执行，突显了该框架在先前方法失效的场景中的有效性。 
